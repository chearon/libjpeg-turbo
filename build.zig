...
fn addArmFiles(b: *std.Build, library: *std.Build.Step.Compile, is64: bool) void {
    ...
    const arch_sources = if (is64) blk: {
        var sources = std.ArrayList([]const u8).init(std.heap.page_allocator);
...
fn addX86Files(b: *std.Build, lib: *std.Build.Step.Compile, target: std.Build.ResolvedTarget) void {
    ...
    var dargs = std.ArrayList([]const u8).init(std.heap.page_allocator);
    defer dargs.deinit();
    ...
    for (sources) |input_file| {
        ...
        nasm_run.addArg(b.fmt("-I{}/", .{root}));
        nasm_run.addArg(b.fmt("-I{}/simd/nasm/", .{root}));
        nasm_run.addArg(b.fmt("-I{}/{s}/", .{root, std.fs.path.dirname(input_file).?}));
